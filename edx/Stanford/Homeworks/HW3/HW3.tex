...
\[
	\text{Size of Good Range} = \alpha n - (1 - \alpha)n = (2\alpha - 1)n = 2\alpha n - n
\]
...
\textbf{Solution:} To prove this, we utilize \textbf{Yao's Minimax Principle}, which allows us to establish a lower bound on the worst-case expected cost of a randomized algorithm by analyzing the average-case cost of a deterministic algorithm over a specific input distribution.

\smallskip

\textbf{Key Idea:} The average performance of a randomized algorithm on its single worst-case input ... is the same as ... The average performance of the best possible deterministic algorithm on a randomly shuffled input.
